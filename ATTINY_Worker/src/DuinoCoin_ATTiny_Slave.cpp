...
void setup() {
  if (SENSOR_EN) {
    ...
    ADCSRA &=
        ~(_BV(ADATE) | _BV(ADIE)); // Clear auto trigger and interrupt enable
...
void do_work() {
  if (working) {
    ...
    if (buffer[0] == 'g') {
      ...
      case 't': // temperature
        if (SENSOR_EN)
          ltoa(getTemperature(), buffer, 8);
        else
          strcpy_P(buffer, ZERO);
...
// #define HTOI(c)
// ((c<='9')?(c-'0'):((c<='F')?(c-'A'+10):((c<='f')?(c-'a'+10):(0))))
...
void HEX_TO_BYTE(char *address, char *hex, uint8_t len) {
  for (uint8_t i = 0; i < len; i++)
    address[i] = TWO_HTOI(hex[2 * i], hex[2 * i + 1]);
}
